const coder = {
  stringToUTF8Buffer: function(string) {
  ...
  codePointsToUTF8Buffer: function(codePoints) {
    ...
    // a code point is represented by four bytes at most
...
const aes256 = {
  ...
  KEY_BYTES: 32,
  KEY_SCHEDULE_ROUNDS: 15,
  KEY_SCHEDULE_BYTES: 60,
  SUBSTITUTE: [
  ...
  encryptBlock: function(state, key) {
    // https://csrc.nist.gov/csrc/media/projects/cryptographic-standards-and-guidelines/documents/aes-development/rijndael-ammended.pdf
    // T-tables from: https://golang.org/src/crypto/aes/const.go#L80
    ...
    const roundKeys = this.computeRoundKeys(key);
    const s = state;
    const k = roundKeys;
    ...
    let s2 = s[2];
    let s3 = s[3];
    for (let i = 0; i < this.KEY_SCHEDULE_ROUNDS - 2; i++) {
      s0 = k[j + 0] ^ (t0[0xff & s[0] >> 24]) ^ (t1[0xff & s[1] >> 16]) ^ (t2[0xff & s[2] >> 8]) ^ t3[0xff & s[3]] >>> 0;
      s1 = k[j + 1] ^ (t0[0xff & s[1] >> 24]) ^ (t1[0xff & s[2] >> 16]) ^ (t2[0xff & s[3] >> 8]) ^ t3[0xff & s[0]] >>> 0;
      s2 = k[j + 2] ^ (t0[0xff & s[2] >> 24]) ^ (t1[0xff & s[3] >> 16]) ^ (t2[0xff & s[0] >> 8]) ^ t3[0xff & s[1]] >>> 0;
      s3 = k[j + 3] ^ (t0[0xff & s[3] >> 24]) ^ (t1[0xff & s[0] >> 16]) ^ (t2[0xff & s[1] >> 8]) ^ t3[0xff & s[2]] >>> 0;
      j += 4;
      s[0]= s0;
      s[1]= s1;
      s[2]= s2;
    ...
    s[0] = k[j + 0] ^ (sub[0xff & s0 >> 24] << 24 | sub[0xff & s1 >> 16] << 16 | sub[0xff & s2 >> 8] << 8 | sub[0xff & s3]);
    s[1] = k[j + 1] ^ (sub[0xff & s1 >> 24] << 24 | sub[0xff & s2 >> 16] << 16 | sub[0xff & s3 >> 8] << 8 | sub[0xff & s0]);
    s[2] = k[j + 2] ^ (sub[0xff & s2 >> 24] << 24 | sub[0xff & s3 >> 16] << 16 | sub[0xff & s0 >> 8] << 8 | sub[0xff & s1]);
    s[3] = k[j + 3] ^ (sub[0xff & s3 >> 24] << 24 | sub[0xff & s0 >> 16] << 16 | sub[0xff & s1 >> 8] << 8 | sub[0xff & s2]);
    return s;
  },

  computeRoundKeys: function(key) {
    const ROUND_CONSTANT = [
      0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40
    ];
    function g(word, round) {
      return ((this.SUBSTITUTE[0xff & word >> 16] ^ ROUND_CONSTANT[round]) << 24) | 
               this.SUBSTITUTE[0xff & word >>  8] << 16 |
               this.SUBSTITUTE[0xff & word >>  0] <<  8 | 
               this.SUBSTITUTE[0xff & word >> 24] <<  0 >>> 0;
    }
    function h(word) {
      return this.SUBSTITUTE[0xff & word >> 24] << 24 |
             this.SUBSTITUTE[0xff & word >> 16] << 16 |
             this.SUBSTITUTE[0xff & word >>  8] <<  8 |
             this.SUBSTITUTE[0xff & word >>  0] >>> 0;
    }
    const KEY_WORDS = 8;
    const w = new Uint32Array(this.KEY_SCHEDULE_BYTES);
    for (let i = 0; i < KEY_WORDS; i++) {
      w[i] = key[i];
    }
    for (let i = 0; i < this.KEY_SCHEDULE_ROUNDS; i++) {
      r = KEY_WORDS * (i + 1);
        w[r    ] = w[r - 8] ^ g.apply(this, [w[r - 1], i]);
        w[r + 1] = w[r - 7] ^   w[r    ];
        w[r + 2] = w[r - 6] ^   w[r + 1]; 
        w[r + 3] = w[r - 5] ^   w[r + 2];
        if (i !== this.KEY_SCHEDULE_ROUNDS - 1) {
          w[r + 4] = h.apply(this, [w[r + 3]]) ^ w[r - 4];
          w[r + 5] =   w[r + 4]  ^ w[r - 3];
          w[r + 6] =   w[r + 5]  ^ w[r - 2]; 
          w[r + 7] =   w[r + 6]  ^ w[r - 1];
        }
...
(function () {
  const string = "aaabbc123😀🏳️‍🌈Ø传";
  const expectedEncoding = Buffer.from(string);
  const encodedString = Buffer.from(coder.stringToUTF8Buffer(string));
  if (expectedEncoding.compare(encodedString) !== 0) {
    throw new Error("UTF-16 to UTF-8 encoding failed");
  } else {
...
(function () {
  ...
  const result = aes256.computeRoundKeys(key);
  for (let i = 0; i < expected.length; i++) {
    if (expected[i] !== result[i]) {
      throw new Error(`key generation failed: word at index ${i} should equal ${roundKeys[i].toString(16)}, but equals ${expectedRoundKeys[i].toString(16)}`);
    }
  }
...
(function () {
  ...
  for (let i = 0; i < expected.length; i++) {
    if (expected[i] !== result[i]) {
      throw new Error(`encryption failed: expected \n${expected[i].toString(16)} instead of \n${result[i].toString(16)}`);
    }
  }
